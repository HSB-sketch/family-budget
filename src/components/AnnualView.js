...
const CAT_COLORS = ['#6366f1','#8b5cf6','#22d3ee','#10b981','#f59e0b','#ef4444','#ec4899','#84cc16','#f97316','#6b7280'];
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          <h2 style={{ fontSize: 22, fontWeight: 700, color: '#f0f0f8', marginBottom: 4 }}>Annual Overview</h2>
          <p style={{ fontSize: 13, color: '#6b7280' }}>Jan – {MONTHS[currentMonth - 1]} {year} · all family members</p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
        ...
        <StatCard
          ...
          accent={ytdBudget > 0 ? (ytdTotal <= ytdBudget ? '#10b981' : '#ef4444') : '#22d3ee'}
        />
      </div>
      ...
      <div style={panel}>
          ...
          <BarChart data={monthlyTotals} margin={{ top: 4, right: 4, left: -16, bottom: 0 }}>
            ...
            <Bar dataKey="spent" name="Spent" radius={[4,4,0,0]}>
              {monthlyTotals.map((m, i) => (
                <Cell key={i} fill={m.over ? '#ef4444' : m.month === currentMonth ? '#22d3ee' : '#6366f1'} />
              ))}
            </Bar>
        ...
        <div style={{ display: 'flex', gap: 16, marginTop: 8, fontSize: 11, color: '#6b7280' }}>
          <span><span style={{ display: 'inline-block', width: 10, height: 10, borderRadius: 2, background: '#6366f1', marginRight: 4 }} />Spent</span>
          ...
          <span><span style={{ display: 'inline-block', width: 10, height: 10, borderRadius: 2, background: '#22d3ee', marginRight: 4 }} />Current month</span>
        </div>
      </div>
      ...
      <div style={panel}>
        ...
        <ResponsiveContainer width="100%" height={180}>
          <LineChart data={cumulativeData} margin={{ top: 4, right: 4, left: -16, bottom: 0 }}>
            ...
            <Line type="monotone" dataKey="cumSpent" name="Spent" stroke="#6366f1" strokeWidth={2.5} dot={{ r: 3, fill: '#6366f1' }} activeDot={{ r: 5 }} />
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              ...
              {monthlyTotals.map((m, i) => {
                ...
                return (
                  <tr key={i} style={{ background: isCurrent ? 'rgba(34,211,238,0.05)' : 'transparent' }}>
                    <td style={td}>
                      <span style={{ color: isCurrent ? '#22d3ee' : '#d1d5db', fontWeight: isCurrent ? 600 : 400 }}>
                        {m.label} {isCurrent ? '←' : ''}
                      </span>
                    </td>
                    ...
                    <td style={td}>
                      {isFuture ? <span style={badge('#4b5563','#1f2937')}>Upcoming</span>
                        ...
                        : variance === null ? <span style={badge('#6b7280','rgba(107,114,128,0.15)')}>No budget</span>
            ...
            <tfoot>
              <tr style={{ borderTop: '1px solid rgba(255,255,255,0.08)' }}>
                <td style={{ ...td, color: '#9ca3af', fontWeight: 700 }}>YTD Total</td>
                <td style={{ ...td, color: '#a5b4fc', fontWeight: 700 }}>${ytdTotal.toFixed(2)}</td>
                <td style={{ ...td, color: '#9ca3af', fontWeight: 700 }}>{ytdBudget > 0 ? `$${ytdBudget.toFixed(2)}` : '—'}</td>
                ...
                <div key={i} style={varRow}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: c.ytdBudget > 0 ? 6 : 0 }}>
                    ...
                    <div style={{ display: 'flex', gap: 10, fontSize: 12 }}>
                      ...
                      {c.ytdBudget > 0 && (
                        <span style={{ color: over ? '#ef4444' : '#6b7280' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(120px,1fr))', gap: 10 }}>
            {memberYTD.map((m) => (
              <div key={m.name} style={{ background: 'rgba(255,255,255,0.03)', border: `1px solid ${m.color}33`, borderRadius: 12, padding: '1rem', textAlign: 'center' }}>
                ...
                <div style={{ fontSize: 12, color: '#9ca3af', marginBottom: 2 }}>{m.name}</div>
...
function StatCard({ label, value, accent }) {
  return (
    <div style={{ background: 'rgba(255,255,255,0.04)', border: `1px solid ${accent}33`, borderRadius: 14, padding: '1rem' }}>
...
const sectionLabel = { fontSize: 11, fontWeight: 700, color: '#6b7280', letterSpacing: '0.08em', marginBottom: 14 };
...
const th = { padding: '8px 10px', textAlign: 'left', fontSize: 11, fontWeight: 700, color: '#6b7280', letterSpacing: '0.05em', borderBottom: '1px solid rgba(255,255,255,0.06)', whiteSpace: 'nowrap' };
const td = { padding: '9px 10px', color: '#9ca3af', borderBottom: '1px solid rgba(255,255,255,0.04)', whiteSpace: 'nowrap' };
